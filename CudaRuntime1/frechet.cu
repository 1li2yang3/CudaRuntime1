...
__global__ void frechet_kernel_wavefront(const Point* t1, const Point* t2, float* results, int n)
{
    ...
    // 将一维的动态共享内存划分为三段，每段长度为 n
    float* dp_prev2 = s_dp;
    float* dp_prev1 = s_dp + n;
    float* dp_curr = s_dp + 2 * n;

    for (int k = 0; k < 2 * n - 1; k++) { // 这里没有虚拟边界，所以k从0到2n-1
        int start_i = max(0, k - n + 1);  // i+j=k, 0<=i<=n-1, 0<=j<=n-1
        int len = min(k, n - 1) - start_i + 1; // 该条对角线上的元素个数
        ...
        // 必须同步，保证所有线程都计算完当前的对角线，再进行指针交换
        __syncthreads();

        // 滚动数组更新指针 (仅仅交换指向共享内存的指针，开销极小)
        float* temp = dp_prev2;
        dp_prev2 = dp_prev1;
        dp_prev1 = dp_curr;
        dp_curr = temp;
...
void launch_frechet_batch_gpu_wavefront(const Point* h_t1, const Point* h_t2, float* h_results, int num_t, int n, float& gpu_time) {
    ...
    // 注意：已经移除了 dp_prev2, dp_prev1, dp_curr 的 cudaMalloc 全局内存分配
    ...
    // 计算动态共享内存的大小：3 个长度为 n 的 float 数组
    size_t shared_mem_size = 3 * n * sizeof(float);
    cudaFuncSetAttribute(frechet_kernel_wavefront, cudaFuncAttributeMaxDynamicSharedMemorySize, shared_mem_size);
    // 启动Kernel，传入 shared_mem_size
